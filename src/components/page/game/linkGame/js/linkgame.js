 ...
 window.onload = function () {
     ...
     var cellSize = 150;
     _img_face_m = 5;//横排的格子数
     _img_face_n = 4;//竖排的格子数
     _img_face_src_width = cellSize;//
     _img_face_src_height = cellSize;
     _img_face_width = cellSize/2;
     _img_face_height = cellSize/2;
     _img_face_radius = 10;
 ...
 function btnNewGame_Click() {
     ...
     if (boardsize != _boardsize) {
         ...
         var _thisW = document.body.clientWidth;
         var _thisB = 2;
         _img_face_width = _thisW/(_board_m+_thisB);
         _img_face_height = _thisW/(_board_m+_thisB);
          _width = (_board_m + _thisB) * _img_face_width;
          _height = (_board_n + _thisB) * _img_face_height;
        //  _width = _board_m * _img_face_width;
        //  _height = _board_n * _img_face_height;
         _board_m_offset = 0;
         _board_n_offset = 0;
         ...
         if (_ctx != null) {
            //  _canvas.width = _width;
            //  _canvas.height = _height;
             _canvas.width = _thisW;
             _canvas.height = _thisW/(_board_m+_thisB)*(_board_n+_thisB);
             _ctx.restore();
             _ctx.save();
 ...
 function OnCanvas_Click(e) {
     ...
     if (_first_p == null) {
         _first_p = board_p;
         draw_face_animation(board_p.X, board_p.Y, 'rgba(40,200,200, 1.0)');
     }
         ...
         if (_board.IsFit(path)) {
             if (_ctx != null) {
                 draw_face_animation(path.End.X, path.End.Y, 'rgba(40,200,200, 1.0)');
 ...
 function draw_time(tms) {
     var x = (_board_m + 2) * _img_face_width / 2 - 90;
     var y = 40;

     _ctx.save();
     _ctx.clearRect(x, -15, 250, _img_face_height+15);
     _ctx.font = "32pt Arial";
     _ctx.fillStyle = "orange";
